...
def create_price_chart(price_data, calls=None, puts=None, exposure_levels_types=[], exposure_levels_count=3, call_color='#00FF00', put_color='#FF0000', strike_range=0.02, use_heikin_ashi=False, highlight_max_level=False, max_level_color='#800080', timezone='America/New_York'):
    ...
    # Filter to display only current day's candles
    display_candles = [candle for candle in all_candles
                       if datetime.fromtimestamp(candle['datetime']/1000, pytz.UTC).astimezone(tz).date() == current_date]

    # Fall back to all candles if no current day data
    ...
    # Set x-axis range to focus on current day's trading session (extended hours)
    # This zooms in to today's activity while still allowing users to pan to see previous days
    current_day_candles = [d for d in dates if d.date() == current_date]

    if current_day_candles:
        # Get the extended hours range for today (4 AM - 8 PM ET)
        et_tz = pytz.timezone('America/New_York')
        today_start_et = et_tz.localize(datetime.combine(current_date, datetime.min.time()).replace(hour=4, minute=0))
        today_end_et = et_tz.localize(datetime.combine(current_date, datetime.min.time()).replace(hour=20, minute=0))

        # Convert to user's selected timezone
        today_start = today_start_et.astimezone(tz)
        today_end = today_end_et.astimezone(tz)

        xaxis_range = [today_start, today_end]
    else:
        # Fallback: if no current day data, show all available data
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            mirror=True,
            domain=[0, 1],
            range=xaxis_range,  # Set initial range to current day (maximized)
            fixedrange=False  # Allow user to zoom and pan to see other days
        ),
        yaxis=dict(
            title='Price',
            title_font=dict(color='#CCCCCC'),
...
<body>
    ...
    <div class="container">
            ...
            <div class="header-top">
                ...
                <div class="controls">
                    ...
                    <div class="settings-control">
                        ...
                        <button id="runScanner" title="Scan all tickers for trading opportunities">🔍 Scanner</button>
                    </div>
                </div>
            </div>
        ...
        <div id="scannerModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 10000; overflow: auto;">
            <div style="background: #1a1a1a; margin: 50px auto; padding: 30px; width: 90%; max-width: 1200px; border-radius: 10px; border: 2px solid #444;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                    <h2 style="color: #fff; margin: 0;">📊 Option Scanner Results</h2>
                    <button onclick="closeScannerModal()" style="background: #ff4444; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; font-size: 16px;">✕ Close</button>
                </div>
                <div id="scannerResults" style="color: #fff;">
    ...
    <script>
        ...
        function initTabEventListeners(tabId) {
            ...
            // Helper to get tab-specific element
            const getElement = (idPrefix) => {
                if (tabId === 0) {
                    return tabContent.querySelector(`#${idPrefix}`);
                } else {
                    return tabContent.querySelector(`#${idPrefix}-tab-${tabId}`);
                }
            };

            // Ticker change listener
            ...
            if (ticker) {
                ...
                // Ticker input listener - stop auto-update when user starts typing
                ticker.addEventListener('input', function(e) {
                    if (activeTabId === tabId && isStreaming) {
                        toggleStreaming();
                    }
                });

                // Ticker keypress listener - start auto-update on Enter
                ticker.addEventListener('keypress', function(e) {
                    if (activeTabId === tabId && e.key === 'Enter') {
            ...
            // Stream toggle listener
            const streamToggle = getElement('streamToggle');
            if (streamToggle) {
            ...
            // Expiry dropdown event listeners
            const expiryDisplay = getElement('expiry-display');
            const expiryOptions = getElement('expiry-options');
            if (expiryDisplay && expiryOptions) {
                expiryDisplay.addEventListener('click', function(e) {
                    e.stopPropagation();
                    if (activeTabId === tabId) {
                        expiryOptions.classList.toggle('open');
                    }
                });
            }

            // Levels dropdown event listeners
            const levelsDisplay = getElement('levels-display');
            const levelsOptions = getElement('levels-options');
            if (levelsDisplay && levelsOptions) {
                levelsDisplay.addEventListener('click', function(e) {
                    e.stopPropagation();
                    if (activeTabId === tabId) {
                        levelsOptions.classList.toggle('open');
                    }
                });
            }

            // Close dropdowns when clicking outside (for this tab)
            document.addEventListener('click', function(e) {
                if (activeTabId === tabId) {
                    const expiryDropdown = tabContent.querySelector('.expiry-dropdown');
                    if (expiryDropdown && !expiryDropdown.contains(e.target)) {
                        expiryOptions.classList.remove('open');
                    }

                    const levelsDropdown = tabContent.querySelector('.levels-dropdown');
            ...
            // Color pickers
            const callColor = getElement('call_color');
            if (callColor) {
                callColor.addEventListener('change', function(e) {
                    if (activeTabId === tabId) {
                        updateData();
                    }
                });
            ...
            const putColor = getElement('put_color');
            if (putColor) {
                putColor.addEventListener('change', function(e) {
                    if (activeTabId === tabId) {
                        updateData();
                    }
                });
            ...
            const maxLevelColor = getElement('max_level_color');
            if (maxLevelColor) {
                maxLevelColor.addEventListener('change', function(e) {
                    if (activeTabId === tabId) {
                        updateData();
                    }
                });
            ...
            // All control inputs that trigger updateData
            const controlIds = [
                'highlight_max_level', 'max_level_mode', 'gex_absolute',
                'strike_range', 'timeframe', 'timezone', 'coloring_mode',
                'exposure_metric', 'levels_count', 'abs_gex_opacity'
            ];

            controlIds.forEach(id => {
                const element = getElement(id);
                if (element) {
                    const eventType = (id === 'strike_range' || id === 'levels_count' || id === 'abs_gex_opacity') ? 'input' : 'change';
                    element.addEventListener(eventType, function() {
                        if (activeTabId === tabId) {
                            // Update strike range display if needed
                            if (id === 'strike_range') {
                                const rangeValue = getElement('strike_range_value');
            ...
            // Chart checkboxes
            tabContent.querySelectorAll('.chart-checkbox input[type="checkbox"]').forEach(checkbox => {
            ...
            tabContent.querySelectorAll('.control-group input[type="checkbox"]').forEach(checkbox => {
                checkbox.addEventListener('change', function() {
                    if (activeTabId === tabId) {
                        updateData();
                    }
            ...
            // Expiry selection buttons
            const selectAllExpiry = getElement('selectAllExpiry');
            if (selectAllExpiry) {
                selectAllExpiry.addEventListener('click', function(e) {
                    e.stopPropagation();
                    if (activeTabId === tabId) {
                        const checkboxes = tabContent.querySelectorAll('.expiry-option input[type="checkbox"]');
            ...
            const clearAllExpiry = getElement('clearAllExpiry');
            if (clearAllExpiry) {
                clearAllExpiry.addEventListener('click', function(e) {
                    e.stopPropagation();
                    if (activeTabId === tabId) {
                        const checkboxes = tabContent.querySelectorAll('.expiry-option input[type="checkbox"]');
                        checkboxes.forEach(checkbox => {
            ...
            // Level checkboxes
            tabContent.querySelectorAll('.levels-option input[type="checkbox"]').forEach(checkbox => {
                checkbox.addEventListener('change', function() {
                    if (activeTabId === tabId) {
                        updateLevelsDisplay();
                        updateData();
            ...
            // Expiry option checkboxes (dynamically added, so use delegation if needed later)
            tabContent.querySelectorAll('.expiry-option input[type="checkbox"]').forEach(checkbox => {
                checkbox.addEventListener('change', function() {
                    if (activeTabId === tabId) {
                        updateExpiryDisplay();
                        updateData();
...
<\\/script></body></html>`);
        ...
        // Note: All color pickers and control input event listeners are now handled in initTabEventListeners()
        ...
        // Note: Levels dropdown and checkbox event listeners are now handled in initTabEventListeners()
            ...
            fetch(`/expirations/${ticker}`)
                ...
                .then(data => {
                    ...
                    data.forEach(date => {
                        ...
                        // Add change event listener (scoped via overridden document methods)
                        checkbox.addEventListener('change', function() {
                            updateExpiryDisplay();
                            updateData();
                        });
        ...
        // Note: All chart checkboxes, control checkboxes, ticker, dropdown, and expiry button
        // event listeners are now handled in initTabEventListeners()

        // Initialize tab management
        initTabManagement();

        // Initialize event listeners for the first tab (tab 0)
        initTabEventListeners(0);
        ...
        // Note: streamToggle event listener is now handled in initTabEventListeners()
        ...
        function getAllTickers() {
            ...
            for (let i = 0; i < tickerSelect.options.length; i++) {
                const value = tickerSelect.options[i].value;
                // Skip special tickers
                if (value !== 'MARKET' && value !== 'MARKET2') {
                    tickers.push(value);
                }
            }
            return tickers;
        ...
        async function runScanner() {
            ...
            resultsDiv.innerHTML = '<div style="text-align: center; padding: 40px;"><div style="font-size: 24px; margin-bottom: 10px;">⏳ Scanning tickers...</div><div style="color: #aaa;">This may take a minute...</div></div>';
            ...
            try {
                ...
                resultsDiv.innerHTML = `<div style="color: #ff4444; text-align: center; padding: 40px;">Error running scanner: ${error.message}</div>`;
            } finally {
                btn.disabled = false;
        ...
        function displayScannerResults(results) {
            ...
            if (!results || results.length === 0) {
                resultsDiv.innerHTML = '<div style="text-align: center; padding: 40px; color: #aaa;">No results found</div>';
                return;
            }

            // Separate by recommendation type
            const calls = results.filter(r => r.recommendation === 'CALL' && r.confidence > 0);
            const puts = results.filter(r => r.recommendation === 'PUT' && r.confidence > 0);
            const neutral = results.filter(r => r.recommendation === 'NEUTRAL' || r.confidence === 0);

            let html = '<div style="display: grid; gap: 20px;">';

            // CALL recommendations
            if (calls.length > 0) {
                html += '<div style="background: #1a3a1a; padding: 20px; border-radius: 8px; border: 2px solid #00aa00;">';
                html += '<h3 style="color: #00ff00; margin-top: 0;">🟢 CALL Opportunities (' + calls.length + ')</h3>';
                html += '<div style="display: grid; gap: 15px;">';

                calls.forEach(result => {
                    html += `<div style="background: #0d1f0d; padding: 15px; border-radius: 5px; border-left: 4px solid #00ff00;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <div style="font-size: 20px; font-weight: bold; color: #00ff00;">${result.ticker}</div>
                            <div style="font-size: 18px; color: #00ff00;">Confidence: ${result.confidence}%</div>
                        </div>
                        <div style="margin-bottom: 8px; color: #aaa;">Price: $${result.metrics.current_price?.toFixed(2) || 'N/A'} | Change: ${result.metrics.price_change_pct?.toFixed(2)}% | RSI: ${result.metrics.rsi?.toFixed(1)}</div>
                        <div style="font-size: 14px; color: #ccc;">
                            ${result.signals.map(s => `<div style="margin: 5px 0;">• ${s}</div>`).join('')}
                        </div>
                    </div>`;
                });
            ...
            // PUT recommendations
            if (puts.length > 0) {
                html += '<div style="background: #3a1a1a; padding: 20px; border-radius: 8px; border: 2px solid #aa0000;">';
                html += '<h3 style="color: #ff0000; margin-top: 0;">🔴 PUT Opportunities (' + puts.length + ')</h3>';
                html += '<div style="display: grid; gap: 15px;">';

                puts.forEach(result => {
                    html += `<div style="background: #1f0d0d; padding: 15px; border-radius: 5px; border-left: 4px solid #ff0000;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <div style="font-size: 20px; font-weight: bold; color: #ff0000;">${result.ticker}</div>
                            <div style="font-size: 18px; color: #ff0000;">Confidence: ${result.confidence}%</div>
                        </div>
                        <div style="margin-bottom: 8px; color: #aaa;">Price: $${result.metrics.current_price?.toFixed(2) || 'N/A'} | Change: ${result.metrics.price_change_pct?.toFixed(2)}% | RSI: ${result.metrics.rsi?.toFixed(1)}</div>
                        <div style="font-size: 14px; color: #ccc;">
                            ${result.signals.map(s => `<div style="margin: 5px 0;">• ${s}</div>`).join('')}
            ...
            // Summary stats
            html += '<div style="background: #2a2a2a; padding: 15px; border-radius: 8px; text-align: center;">';
            html += `<div style="font-size: 16px;">Total Scanned: ${results.length} | Calls: ${calls.length} | Puts: ${puts.length} | Neutral: ${neutral.length}</div>`;
            html += '</div>';
        ...
        // Note: Ticker input/keypress event listeners are now handled in initTabEventListeners()
    </script>
</body>
</html>
    ''')
...
def analyze_ticker_for_scanner(ticker, timezone='America/New_York'):
    """
    Analyze a single ticker and determine if calls or puts should be bought.

    Returns dict with:
    - recommendation: 'CALL', 'PUT', or 'NEUTRAL'
    - confidence: 0-100 score
    - signals: list of bullish/bearish signals detected
    ...
    try:
        ...
        # Get price history (5 days of 1-min candles)
        price_data = get_price_history(ticker, timeframe=1, timezone=timezone)
        ...
        today_candles = [c for c in candles
                        if datetime.fromtimestamp(c['datetime']/1000, pytz.UTC).astimezone(tz).date() == current_date]

        if not today_candles or len(today_candles) < 10:
            today_candles = candles[-50:]  # Use last 50 candles if today has insufficient data
        ...
        recent_momentum = ((closes[-1] - closes[-min(5, len(closes))]) / closes[-min(5, len(closes))]) * 100

        if price_change_pct > 0.5:
            signals.append(f'📈 Strong uptrend today (+{price_change_pct:.2f}%)')
            bullish_score += 20
        elif price_change_pct < -0.5:
            signals.append(f'📉 Strong downtrend today ({price_change_pct:.2f}%)')
            bearish_score += 20

        if recent_momentum > 0.2:
            signals.append(f'⚡ Recent upward momentum (+{recent_momentum:.2f}%)')
            bullish_score += 15
        elif recent_momentum < -0.2:
            signals.append(f'⚡ Recent downward momentum ({recent_momentum:.2f}%)')
            bearish_score += 15

        # 2. RSI ANALYSIS
        rsi = calculate_rsi(closes)
        if rsi < 30:
            signals.append(f'🔥 Oversold (RSI: {rsi:.1f}) - Potential bounce')
            bullish_score += 25
        elif rsi > 70:
            signals.append(f'❄️ Overbought (RSI: {rsi:.1f}) - Potential pullback')
            bearish_score += 25
        elif rsi > 50 and rsi < 70:
            signals.append(f'💪 Bullish momentum (RSI: {rsi:.1f})')
            bullish_score += 10
        elif rsi < 50 and rsi > 30:
            signals.append(f'📊 Bearish momentum (RSI: {rsi:.1f})')
            bearish_score += 10
        ...
        if volume_ratio > 1.5:
            if price_change_pct > 0:
                signals.append(f'📊 High volume on up move (Vol: {volume_ratio:.1f}x avg)')
                bullish_score += 15
            else:
                signals.append(f'📊 High volume on down move (Vol: {volume_ratio:.1f}x avg)')
                bearish_score += 15
        ...
        # Check if price is near high or low
        distance_from_high = ((current_high - closes[-1]) / closes[-1]) * 100
        distance_from_low = ((closes[-1] - current_low) / closes[-1]) * 100

        if distance_from_high < 0.3:
            signals.append(f'⬆️ Near day high (${current_high:.2f})')
            bullish_score += 10
        elif distance_from_low < 0.3:
            signals.append(f'⬇️ Near day low (${current_low:.2f})')
            bearish_score += 10
        ...
        if len(closes) >= 20:
            ...
            if second_half_high > first_half_high and second_half_low > first_half_low:
                signals.append('📈 Higher highs & higher lows - Strong uptrend')
                bullish_score += 15
            elif second_half_high < first_half_high and second_half_low < first_half_low:
                signals.append('📉 Lower highs & lower lows - Strong downtrend')
                bearish_score += 15

        # 6. VOLATILITY ANALYSIS
        volatility = (price_range / closes[0]) * 100
        if volatility > 2:
            signals.append(f'⚠️ High volatility ({volatility:.2f}%) - Trade with caution')
...
def run_scanner():
    """Run option scanner on selected tickers"""
    try:
        ...
        for ticker in tickers:
            ...
            result = analyze_ticker_for_scanner(formatted_ticker)
